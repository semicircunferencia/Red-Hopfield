...
# Parámetros
# ========================================
file_in = "patron_1.dat" # Nombre del fichero de datos
file_out = "patron_1" # Nombre del fichero de salida (sin extensión)
...
def add_commas_to_string(string):
    """Agrega una coma entre cada carácter de un string, omitiendo "\n"."""
    new_string = ""
    for i, char in enumerate(string):
        if char == "\n":
            if i == len(string) - 1:  # el salto de línea es el último carácter
                new_string += char
            else:  # hay más caracteres después del salto de línea
                new_string = new_string[:-1]  # elimina la última coma
                new_string += char
        else:
            new_string += char + ","
    return new_string[:-1]  # elimina la última coma agregada
...
for frame_data_str in data_str.split("\n\n"):
    ...
    frame_data_str = add_commas_to_string(frame_data_str)
